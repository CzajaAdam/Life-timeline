<?php
    ...
    try {
        ...
        $locationStmt = $db->prepare("SELECT * FROM `event_locations` WHERE event_id = :event_id ORDER BY created_at DESC");
        $locationStmt->execute(['event_id' => $eventId]);
        $location = $locationStmt->fetchAll(PDO::FETCH_ASSOC);
