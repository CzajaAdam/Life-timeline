<?php
    ...
    try {
        ...
        $stmt = $db->prepare("DELETE FROM `event_notes` WHERE `id` = :id AND `created_by` = :user_id LIMIT 1");
        $stmt->execute([
            'id' => $noteId,
        ...
        header('Location: ../../../event-page.php?id=' . $eventId . '&success=Note+deleted+successfully');
        exit();
