...
<body class="bg-charcoal-950 relative">
    <div class="container mx-auto flex flex-col min-h-screen p-4 pb-24">
        ...
        <section class="flex flex-col justify-start grow pt-2 items-center px-4">
            ...
            <!-- Create Event Form -->
            <form class="hidden flex-col p-8 rounded-xl text-charcoal-50 bg-charcoal-900 border border-charcoal-800 shadow-2xl w-full max-w-md space-y-4" id="create-event-form" action="src/events/create.php" method="POST">
                ...
                <div class="relative flexi-datepicker">
                    <label class="block text-charcoal-300 text-sm mb-1 font-medium">
                        Event Date
                    </label>
                ...
                <div class="relative">
                    ...
                    <!-- Dropdown Button -->
                    <button type="button" id="icon-dropdown-btn" class="w-full p-3 cursor-pointer rounded-lg bg-charcoal-800 border border-charcoal-700 flex items-center justify-between hover:bg-charcoal-750 focus:outline-none focus:ring-2 focus:ring-caleadon-500 focus:border-transparent transition">
                        <span class="flex items-center gap-3">
                            <span id="selected-icon-name" class="text-charcoal-500">Select Icon</span>
                        </span>
                    ...
                    <div id="icon-grid" class="hidden absolute left-1/2 -translate-x-1/2 w-96 z-10 mt-2 bg-charcoal-800 border border-charcoal-700 rounded-lg shadow-lg p-3">
                        <div class="grid grid-cols-6 gap-2">
                            <?php foreach ($icons as $key => $icon): ?>
                                <button type="button" class="cursor-pointer icon-option w-12 h-12 bg-charcoal-700 hover:bg-charcoal-600 rounded transition-colors flex items-center justify-center text-2xl text-charcoal-300 <?php echo (($key == 0) ? 'selected ring-2 ring-caleadon-500' : '')?>" data-value="<?php echo $icon['value']?>" data-unicode="<?php echo $icon['unicode']?>"><?php echo $icon['unicode']?></button>
                            <?php endforeach; ?>
                        </div>
                    </div>
                </div>
